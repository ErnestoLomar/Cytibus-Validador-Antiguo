...
#                ruta = idRuta[0]
        ...
        if accion == 8:
                ...
                c = conn.cursor()
                c.execute(" SELECT idNegra, csn FROM negra WHERE enviado = 0 LIMIT 1")
...
#                    idUni = data[1]
                ...
                con = sqlite3.connect(cdDbF)
                c = con.cursor()
                c.execute(" SELECT idFotos, csn FROM fotos WHERE enviado = 0 LIMIT 1")
                datosFotos = c.fetchone()
                c.close()
                con.close()
        ...
        if accion == 10:
                ...
                c = conn.cursor()
                c.execute(" SELECT idSensor, fechaHora FROM sensor WHERE enviado = 0 LIMIT 1")
...
#                    idUni = data[1]
        ...
        if accion == 11:
                ...
                c = conn.cursor()
                c.execute(" SELECT iCsn, csn FROM tag WHERE enviado = 0 LIMIT 1")
                datosEV2 = c.fetchone()
                c.close()
                conn.close()
...
#                    idUni = data[1]
    ...
    def procesoDeEnvioFoto(self, dato, idActualizar, accion, salgo, csnFoto, fotoConfirmar):
        ...
        while(salgo != 1):
            ...
            buffer_string = ''
            while romper != 1:
                buffer_string = buffer_string + self.ser.read(ser.inWaiting())
                if '\n' in buffer_string:
    ...
    def procesoDeEnvio(self, dato, idActualizar, accion, salgo):
        ...
        while(salgo != 1):
            ...
            stRead = ""
            i=0
            while (i < 1024) and (stRead[-3:] != "RDY") and (stRead[-5:] != "ERROR") and (stRead[-9:] != "SEND FAIL") and (stRead[-7:] != 'SEND OK') and (stRead[-1:] != ">"):
                stRead += str(self.ser.read(1))
                i += 1
...
#            self.inicializarTodo()
    ...
    def obtenerCoordenadaGPS(self):
        '''
            ...
            ##############################################################
        '''
#        self.parent.lblMsg.setStyleSheet('QLabel { font-size: 75pt; font-family: \
#            Arial; color:Blue}')
        ...
        while (stRead[-2:] != "OK") and not ((stRead[-10:] >= "ERROR: 501") and (stRead[-10:] <= "ERROR: 549")):
            stRead += str(self.ser.read(1))
        print "QGPSLOC: "+stRead
        #self.parent.lblMsg.setText("QGPSLOC: "+stRead)
        #time.sleep(2)
